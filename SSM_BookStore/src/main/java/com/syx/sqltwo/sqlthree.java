...
public class sqlthree {
    public static boolean isUser(userinfo userinfor) throws SQLException, ClassNotFoundException {
        ...
        try {
            ...
            String sql = "select * from  u_2_4_3_2.denglu_two";
            // 连接数据库
            conn = DriverManager.getConnection(url, "u_2_4_3_2", "O4b13#");
            // 建立Statement对象
            ...
            while (rs.next()) {
                ...
                if (userinfor.getUsername().equals(usn) && userinfor.getPassword().equals(pas) && userinfor.getZhandian().equals(z) &&
                    ...
                    flag = true;
                    DataSourceUtil.setDataSource("datasource_sqlserver_u_2_4_3_2");
                }
            }
        }catch (Exception e) {
