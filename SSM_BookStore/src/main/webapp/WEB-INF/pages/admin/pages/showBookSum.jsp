    ...
    <section class="page-shop-sidebar left--sidebar bg--white section-padding--lg">
        <div class="container" style="padding: 20px;border: 1px #ddd solid;">
            <div class="row">
                ...
                <div class="span12" style="margin:2% 5%">
                    <%--按照书籍类型显示--%>
                    <form action="/public/showBookSumByType" method="post">
                        <%if ((Integer) request.getAttribute("code") == 0) {%>
                        <span style="color: red;font-weight: bold">${error}</span>
                        <%}%>
                        <input type="text" name="bookType" placeholder="输入查询图书类型"/>
                        ...
                        <div class="row-fluid">
                            <div class="span12">
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <%if ((Integer) request.getAttribute("code") == 1) {%>
                                    <%List<AdminBook> list = (List<AdminBook>) session.getAttribute("data");%>
                                    <%for (int i = 0; i < list.size(); i++) {%>
                                    <%AdminBook adminBook = list.get(i);%>
                                    <tr>
                                        <td>
                                            <%=adminBook.getId_book()%>
                                        </td>
                                        <td>
                                            <%=adminBook.getId_seller()%>
                                        </td>
                                        <td>
                                            <%=adminBook.getName__book()%>
                                        </td>
                                        <td>
                                            <%=adminBook.getPrice_book()%>
                                        </td>
                                        <td>
                                            <%=adminBook.getPress_book()%>
                                        </td>
                                        <td>
                                            <%=adminBook.getTime_book()%>
                                        </td>
                                        <td>
                                            <%=adminBook.getSum__book()%>
                                        </td>
                                        <td>
                                            <%=adminBook.getType_book()%>
                                        </td>
